...
subtitle: "Part IIA: Linear Models: mod3"
...
set.seed (8131999)
### All pair-wise interactions between base continuous features. 
mod3 <- train(logit_response ~ (.)^2, 
                    data = bf_df,
                    method = "lm",
                    metric = my_metric,
                    trControl = my_ctrl)
...
#mod3$betas %>% dotplot (metric = "RMSE")
#mod3$betas %>% dotplot (metric = "R-squared")
#mod3$betas %>% dotplot (metric = "MAE")
mod3 %>% coefplot::coefplot () + theme_bw() + theme(legend.position='none')
```


```{r, save_model}
mod3 %>% readr::write_rds("linear_mod3_saved.rds")
```

```{r, reload_mod3}
re_load_mod3 <- readr::read_rds("linear_mod3_saved.rds")
```

```{r, show_reload_summary}
re_load_mod3 %>% summary()
```

```{r, check_mod3_equal}
all.equal( mod3, re_load_mod3)
```