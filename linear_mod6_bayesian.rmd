...
subtitle: "Linear Models: Part II b: bayesian mod6"
...
```{r, make_cv_splits}
set.seed(1066)
cv_folds <- vfold_cv(ef_df, v = 5, repeats = 5, strata = logit_response)
cv_folds
```

### Apply Bayesian fitting for mod6

```{r}
mod_6_bayesian <- stan_lm(logit_response ~ (.)^2, 
...
pred_y <- predict(mod_6_bayesian, efm_df)
...
```{r, save_model}
mod_6_bayesian %>% readr::write_rds("linear_mod6_bayesian_saved.rds")
```
...
```{r, check_mod6_bayesin_equal}
all.equal(mod_6_bayesian, re_load_mod6_bayesian)
```