---
title: "Part 4: Regression Models - Model 4"
...
##4. Another linear model of your choice from Part ii)
  
Model 9: spline of z interacted with m

```{r}

cv_folds_mod9 <- vfold_cv(cefm_df, v = 5, repeats = 3, strata = Class)
cv_folds_mod9

glm_spec_mod9 <- logistic_reg() %>% set_engine("glm")

bp_stan_mod9 <- recipe(Class~., data = cefm_df) %>%
  step_dummy(m) %>%
  step_center(all_predictors()) %>% 
  step_scale(all_predictors()) %>%
  step_interact(terms=~contains("z"):starts_with("m")) %>%
  prep(verbose=TRUE)

glm_add_wflow_mod9 <- workflow() %>% 
  add_model(glm_spec_mod9) %>% 
  add_recipe(bp_stan_mod9)

glm_add_res_mod9 <- glm_add_wflow_mod9 %>% 
  fit_resamples(cv_folds_mod9,
                metrics = my_metrics,
                control = control_resamples(save_pred = TRUE))

glm_add_res_mod9$.notes[[1]]

glm_add_res_mod9 %>% collect_metrics()

``` 


The 70% accuracy is slightly better than the empirical event proportion of about 64%.

```{r}
glm_add_res_mod9 %>% collect_predictions() %>% 
  conf_mat(truth = Class, estimate = .pred_class)
```

```{r}

glm_add_res_mod9 %>% collect_predictions(summarize = FALSE) %>% 
  group_nest(id, id2) %>% 
...
```{r}
glm_add_res_mod9 %>% readr::write_rds("part_4_model_4.rds")
```

```{r}
re_load_p4_m4 <- readr::read_rds("part_4_model_4.rds")
re_load_p4_m4 %>% summary()
```
